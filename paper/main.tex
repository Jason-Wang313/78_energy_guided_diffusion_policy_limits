...
\usepackage{url}
\usepackage{booktabs}
\title{Energy-Guided Diffusion Policy Limits: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about diffusion and energy-based robot policies. The original research bet was: Identify when energy guidance changes reachable behavior rather than only re-ranking samples. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item Muscle Coordination Is Habitual Rather than Optimal (2012)
\item Contrastive Energy Prediction for Exact Energy-Guided Diffusion Sampling in Offline Reinforcement Learning (2023)
\item Prompt-Guided Large Language Model for Estimating the State of Charge of Containerized Lithium-Ion Battery Energy Storage Systems (n.d.)
\item DPTG: diffusion policy with tactile feasibility guidance (2026)
\item Guided Discovery of New Behaviors using Diffusion Policies (2026)
\end{itemize}
