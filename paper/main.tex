        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Energy-Guided Diffusion Policy Limits: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study diffusion and energy-based robot policies. The submission-hardened claim is intentionally narrow: Identify when energy guidance changes reachable behavior rather than only re-ranking samples. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Energy guided diffusion policy limits should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.409 $\pm$ 0.012 & 0.435 $\pm$ 0.019 & 0.316 $\pm$ 0.014 \\
Data-augmented WAM & 0.474 $\pm$ 0.018 & 0.514 $\pm$ 0.014 & 0.410 $\pm$ 0.012 \\
Scalar uncertainty & 0.556 $\pm$ 0.012 & 0.553 $\pm$ 0.021 & 0.492 $\pm$ 0.019 \\
Ensemble disagreement & 0.588 $\pm$ 0.018 & 0.608 $\pm$ 0.017 & 0.537 $\pm$ 0.014 \\
Conformal risk filter & 0.597 $\pm$ 0.024 & 0.627 $\pm$ 0.009 & 0.562 $\pm$ 0.009 \\
Proposed branch mechanism & 0.690 $\pm$ 0.021 & 0.688 $\pm$ 0.015 & 0.649 $\pm$ 0.012 \\
Oracle branch upper bound & 0.758 $\pm$ 0.019 & 0.755 $\pm$ 0.010 & 0.734 $\pm$ 0.012 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.671 $\pm$ 0.016 & all components \\
Minus branch atlas & 0.570 $\pm$ 0.021 & removes explicit unrealized futures \\
Minus tail risk & 0.581 $\pm$ 0.012 & uses mean predicted value only \\
Minus diagnostic probe & 0.589 $\pm$ 0.015 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.599 $\pm$ 0.015 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/78.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Energy-Guided Diffusion Policy Limits is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
